...
MIN_H2H_WINS = int(float(os.getenv("MIN_H2H_WINS", "7")))
MIN_EV_PCT = float(os.getenv("MIN_EV_PCT", "2"))

FD_HEADERS = {"X-Auth-Token": FD_TOKEN} if FD_TOKEN else {}

SESSION = requests.Session()
SESSION.headers.update({"User-Agent": "github-actions-football-fast-valuebets/1.0"})
    ...
    for attempt in range(retries):
        try:
            if is_fd:
                ...
                if not ODDS_ENABLED:
                    raise RuntimeError("The Odds API désactivée pour ce run.")
                throttle_odds_api()
            ...
            if status == 429:
                if is_fd:
                    ...
                    print(f"[WARN] Limite football-data atteinte. Nouvelle tentative dans {delay}s.")
                    time.sleep(delay)
                    last_error = error
                    continue
                ...
                if is_odds:
                    ODDS_429_STREAK += 1
                    delay = 3 + attempt * 2
                    print(f"[WARN] The Odds API rate-limited ({ODDS_429_STREAK}/{ODDS_429_MAX_STREAK}), attente {delay}s.")
                    time.sleep(delay)
                    if ODDS_429_STREAK >= ODDS_429_MAX_STREAK:
                        ODDS_ENABLED = False
                        print("[INFO] The Odds API désactivée pour ce run.")
                        raise RuntimeError("The Odds API désactivée après 429 persistants.")
                    last_error = error
                    continue
...
def get_available_competitions():
    data = request_json(f"{BASE_FD}/competitions", headers=FD_HEADERS)
    preferred = []
...
def summarize_h2h(h2h_matches, current_home, current_away):
    ...
    if home_wins >= MIN_H2H_WINS and home_wins > away_wins:
        dominant_team = current_home
        dominant_wins = home_wins
        dominant_losses = away_wins
    elif away_wins >= MIN_H2H_WINS and away_wins > home_wins:
...
def estimate_probabilities_fast(match, h2h_summary):
    home_xg = 1.35
    away_xg = 1.10

    if h2h_summary["h2h_matches"] > 0:
        h2h_bias = (h2h_summary["home_wins"] - h2h_summary["away_wins"]) / h2h_summary["h2h_matches"]
...
def get_odds(match):
    ...
    code = match.get("_competitionCode")
    sport_key = ODDS_SPORTS_MAP.get(code)
    if not sport_key:
        return {}
    ...
    try:
        ...
        print(f"[WARN] Odds ignorées pour {match['homeTeam']['name']} vs {match['awayTeam']['name']}: {error}")
        return {}
...
def build_rows(matches):
    ...
    for idx, match in enumerate(matches, start=1):
        ...
        print(f"[INFO] Analyse {idx}/{len(matches)} : {home_team} vs {away_team}")

        try:
            date_local, time_local = utc_to_paris(match["utcDate"])
            h2h = summarize_h2h(get_h2h(match["id"]), home_team, away_team)

            if not h2h["dominant_team"]:
                continue

            probabilities = estimate_probabilities_fast(match, h2h)
            odds = get_odds(match)
            value_bet = calculate_value_bet(probabilities, odds) if odds else None
            ...
            rows.append({
                ...
                "recent_h2h": json.dumps(h2h["recent_h2h"], ensure_ascii=False),
                ...
                "value_bet_side": value_bet["side"] if value_bet else None,
                "value_bet_probability_pct": value_bet["probability_pct"] if value_bet else None,
                "value_bet_odds": value_bet["odds"] if value_bet else None,
                "value_bet_edge_pct": value_bet["edge_pct"] if value_bet else None,
                "value_bet_ev_pct": value_bet["ev_pct"] if value_bet else None
            })

        except Exception as error:
            print(f"[WARN] Analyse impossible {home_team} vs {away_team}: {error}")
...
def render_discord_message(df):
    if df.empty:
        return "⚠️ Aucun match avec domination H2H forte trouvé sur la période."
    ...
    for row in df.head(10).itertuples():
        ...
        lines.append(f"👑 Dominant : {row.dominant_team}")
        lines.append(f"📚 H2H : {row.dominant_wins}V | {row.draws}N | {row.dominant_losses}D")

        if pd.notna(row.value_bet_side):
            lines.append(
                f"💸 Value bet : {row.value_bet_side} | "
                f"cote {row.value_bet_odds} | EV {row.value_bet_ev_pct}% | {row.bookmaker}"
            )
...
def send_discord(message):
    ...
    for idx, chunk in enumerate(chunks[:5], start=1):
        try:
            response = requests.post(DISCORD_WEBHOOK_URL, json={"content": chunk}, timeout=20)
...
def main():
    ...
    print(f"[INFO] {len(competition_codes)} compétitions exploitables détectées")
    ...
    if not df.empty:
        df = df.sort_values(["date_local", "time_local", "competition", "homeTeam"])

    df.to_csv(OUTPUT_DIR / "football_fast_valuebets.csv", index=False)
    df.to_json(OUTPUT_DIR / "football_fast_valuebets.json", orient="records", force_ascii=False, indent=2)
